...
```
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_route_monitor_operator/<PR_NUMBER>/pull-ci-openshift-route-monitor-operator-master-lint/<BUILD_ID>
```
...
**Job History**: https://prow.ci.openshift.org/?repo=openshift%2Froute-monitor-operator
...
```bash
# View recent PR jobs
curl -s "https://prow.ci.openshift.org/?repo=openshift%2Froute-monitor-operator&type=presubmit" | grep -E "pull-ci-openshift-route-monitor-operator"
...
- **Pull request jobs**: `gs://test-platform-results/pr-logs/pull/openshift_route_monitor_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]`
- **Periodic jobs**: `gs://test-platform-results/logs/[JOB_NAME]/[JOB_ID]`

**Viewing logs via web**:
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_route_monitor_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]
```
...
- `pull-ci-openshift-route-monitor-operator-master-e2e-binary-build-success` - E2E binary build verification
- `pull-ci-openshift-route-monitor-operator-master-coverage` - Code coverage analysis (with Codecov)
- `pull-ci-openshift-route-monitor-operator-master-lint` - Linting checks
- `pull-ci-openshift-route-monitor-operator-master-test` - Unit tests
- `pull-ci-openshift-route-monitor-operator-master-validate` - Validation checks

**Tekton Pipelines** (configured in `.tekton/`):
- `route-monitor-operator-pull-request` - Main PR pipeline (docker build with OCI-TA)
- `route-monitor-operator-e2e-pull-request` - E2E testing pipeline
- `route-monitor-operator-pko-pull-request` - PKO (Package Operator) pipeline
...
```text
https://prow.ci.openshift.org/?repo=openshift%2Froute-monitor-operator&type=presubmit
```
...
- Configuration: `ci-operator/config/openshift/route-monitor-operator/openshift-route-monitor-operator-master.yaml`
- Runs: lint, test, validate, coverage, e2e-binary-build
- Uses Codecov for coverage reporting (secret: `route-monitor-operator-codecov-token`)
...
- Builds container images to Quay (route-monitor-operator-tenant)
...
# Option 2: Use gh CLI to find URLs
gh pr checks <PR_NUMBER> | grep "pull-ci-openshift-route-monitor-operator"
...
# https://prow.ci.openshift.org/?repo=openshift%2Froute-monitor-operator

# Or use platform-specific command:
# macOS: open "https://prow.ci.openshift.org/?repo=openshift%2Froute-monitor-operator"
# Linux: xdg-open "https://prow.ci.openshift.org/?repo=openshift%2Froute-monitor-operator"
# Windows: start "https://prow.ci.openshift.org/?repo=openshift%2Froute-monitor-operator"

# View specific PR on Prow (replace <PR_NUMBER>)
# https://prow.ci.openshift.org/?repo=openshift%2Froute-monitor-operator&type=presubmit&pull=<PR_NUMBER>
```
...
- Prow jobs: `pull-ci-openshift-route-monitor-operator-master-*`
- Tekton jobs: Usually show as "Tekton" or pipeline names in PR checks
- Verify repo name format in Prow: `openshift_route_monitor_operator` (underscore, not dash)
...
- Common issues:
  ...
  - Auth failures → Secret configuration in `route-monitor-operator-tenant` namespace
- Local validation:
  ```bash
...
- Location: `ci-operator/config/openshift/route-monitor-operator/openshift-route-monitor-operator-master.yaml`
- Update process: Submit PR to openshift/release repository
- Auto-generated jobs in: `ci-operator/jobs/openshift/route-monitor-operator/`

**Tekton Pipelines** (in this repo):
- Location: `.tekton/` directory
- Files:
  - `route-monitor-operator-pull-request.yaml` - Main PR pipeline
  - `route-monitor-operator-push.yaml` - Post-merge pipeline
  - `route-monitor-operator-e2e-pull-request.yaml` - E2E testing
  - `route-monitor-operator-pko-pull-request.yaml` - PKO validation
...
- Secret: `route-monitor-operator-codecov-token` (stored in Prow)
- Generate coverage locally: `make coverage`
- Coverage runs on PRs and post-merge (`publish-coverage`)
- Dashboard: Check Codecov for route-monitor-operator
