...
tools: Bash(make lint), Bash(make rbac-wildcard-check), Bash(gitleaks detect *), Bash(prek run gitleaks *), Bash(golangci-lint run --enable gosec *), Read, Grep
model: sonnet
---
...
# Individual checks
gitleaks detect --source . --verbose
golangci-lint run --enable gosec --config=.golangci.yaml
...
- `controllers/*/` (reconciliation logic with cluster access)
- `pkg/rhobs/` (OIDC auth client)
...
Example: [HIGH] [SECRET] pkg/rhobs/client.go:42: Hardcoded API key detected
