...
tools: Bash(make go-build), Bash(make go-test), Bash(make lint), Bash(make docker-build), Bash(prek run *), Bash(boilerplate/_lib/container-make *), Bash(gitleaks detect *), Bash(go env *), Bash(python3 .claude/skills/prow-ci/fetch_prow_artifacts.py *), Bash(python3 .claude/skills/prow-ci/analyze_failure.py *), Read, Grep, WebFetch, WebSearch
model: sonnet
---
...
- `route-monitor-operator-pull-request.yaml`: PR validation
- `route-monitor-operator-push.yaml`: Main branch builds
- `route-monitor-operator-pko-pull-request.yaml`: PKO validation
- `route-monitor-operator-pko-push.yaml`: PKO deployment
- `route-monitor-operator-e2e-pull-request.yaml`: E2E testing pipeline
- `route-monitor-operator-e2e-push.yaml`: E2E post-merge pipeline

**Pipeline Stages:**
1. Checkout code
2. Build container image
3. Run linting (golangci-lint with gosec)
4. Run unit tests
...
# tkn pipeline validate -f .tekton/route-monitor-operator-pull-request.yaml
...
```text
CI Status: FAILING
Pipeline: route-monitor-operator-pull-request
Stage: golangci-lint
Error: Exit code 1

Local Reproduction:
  make lint
  # Output shows gosec findings in controllers/
