...
// isHCPAvailable checks if the HostedControlPlane has the Available condition
// set to True. Returns (true, "") if available, or (false, reason) if not.
func isHCPAvailable(hcp *hypershiftv1beta1.HostedControlPlane) (bool, string) {
	condition := apimeta.FindStatusCondition(hcp.Status.Conditions, string(hypershiftv1beta1.HostedControlPlaneAvailable))
	if condition == nil {
		return false, "AvailableConditionNotFound"
	}
