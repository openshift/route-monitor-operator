...
func (r *HostedControlPlaneReconciler) deployRHOBSAgentResources(ctx context.Context, log logr.Logger, hostedcontrolplane *hypershiftv1beta1.HostedControlPlane) (requeueAfter time.Duration, err error) {
	if r.RHOBSConfig.ProbeAPIURL != "" {
		...
		if err != nil {
			if rhobs.IsNon200Error(err) {
				return rhobsAPIRetryTimeout, nil
			}
			err = fmt.Errorf("failed to deploy RHOBS probe: %w", err)
			return
		}
	}
	return
