...
import (
	...
	blackboxexportermocks "github.com/openshift/route-monitor-operator/pkg/util/test/generated/mocks/blackboxexporter"
...
var _ = Describe("Routemonitor", func() {
	...
	var (
		...
		mockBlackboxExporter *blackboxexportermocks.MockBlackBoxExporter
	...
	BeforeEach(func() {
		...
		mockBlackboxExporter = blackboxexportermocks.NewMockBlackBoxExporter(mockCtrl)
		mockUtils = controllermocks.NewMockUtilInterface(mockCtrl)
	...
	//--------------------------------------------------------------------------------------
	// 		EnsureRouteMonitorAndDependenciesAbsent
	//--------------------------------------------------------------------------------------
	Describe("EnsureRouteMonitorAndDependenciesAbsent", func() {
		...
		JustBeforeEach(func() {
			...
			mockUtils.EXPECT().DeleteServiceMonitorDeployment(gomock.Any(), gomock.Any(), gomock.Any()).
				Times(deleteServiceMonitorDeployment.CalledTimes).
				Return(deleteServiceMonitorDeployment.ErrorResponse)

			mockUtils.EXPECT().DeletePrometheusRuleDeployment(gomock.Any(), gomock.Any(), gomock.Any()).
				Times(deletePrometheusRuleDeployment.CalledTimes).
				Return(deletePrometheusRuleDeployment.ErrorResponse)

			// act
			res, err = routeMonitorReconciler.EnsureRouteMonitorAndDependenciesAbsent(routeMonitor)
		})
		...
		Describe("ShouldDeleteBlackBoxExporterResources instructs to delete", func() {
			...
			When("func EnsureFinalizerAbsent fails unexpectedly", func() {
				BeforeEach(func() {
					...
					mockUtils.EXPECT().DeleteFinalizer(gomock.Any()).Return(true)
					mockUtils.EXPECT().UpdateReconciledCR(gomock.Any(), gomock.Any(), gomock.Any()).Return(utilreconcile.RequeueOperation(), consterror.CustomError)
				})
				It("should bubble up the error", func() {
					Expect(err).To(HaveOccurred())
					Expect(err).To(MatchError(consterror.CustomError))
			...
			When("all deletions happened successfully", func() {
				BeforeEach(func() {
					...
					mockUtils.EXPECT().DeleteFinalizer(gomock.Any()).Return(true)
					mockUtils.EXPECT().UpdateReconciledCR(gomock.Any(), gomock.Any(), gomock.Any())
				})
		...
		When("ShouldDeleteBlackBoxExporterResources instructs to keep the BlackBoxExporter", func() {
			...
			When("the resource has a finalizer but 'Update' succeeds", func() {
				BeforeEach(func() {
					mockUtils.EXPECT().DeleteFinalizer(gomock.Any()).Return(true)
					mockUtils.EXPECT().UpdateReconciledCR(gomock.Any(), gomock.Any(), gomock.Any()).Return(utilreconcile.StopOperation(), nil)
				})
			...
			When("resorce has no finalizer", func() {
				BeforeEach(func() {
					...
					mockUtils.EXPECT().DeleteFinalizer(gomock.Any()).Return(false)
				})
	...
	Describe("EnsureRouteURLExists", func() {
		...
		When("func Update fails unexpectedly", func() {
			...
			BeforeEach(func() {
				...
				mockUtils.EXPECT().UpdateReconciledCR(gomock.Any(), gomock.Any(), gomock.Any()).Times(1).Return(utilreconcile.RequeueOperation(), consterror.CustomError)
		...
		When("the Route has too many Ingress", func() {
			...
			BeforeEach(func() {
				...
				mockUtils.EXPECT().UpdateReconciledCR(gomock.Any(), gomock.Any(), gomock.Any()).Times(1).Return(utilreconcile.StopOperation(), nil)
			})
			JustBeforeEach(func() {
				expectedRouteMonitor.Status.RouteURL = firstRouteURL
			})
	...
	Describe("EnsurePrometheusRuleResourceExists", func() {
		...
		Describe("The RouteMonitor settings are INVALID", func() {
			BeforeEach(func() {
				mockUtils.EXPECT().AreMonitorSettingsValid(routeMonitor.Status.RouteURL, routeMonitor.Spec.Slo).Return(false, customerrors.NoHost, "").Times(1)
			})
			...
			Describe("It deletes existing PrometheusRules", func() {
				...
				When("the PrometheusRule deletion fails", func() {
					BeforeEach(func() {
						mockUtils.EXPECT().DeletePrometheusRuleDeployment(gomock.Any(), gomock.Any(), routeMonitor.Status.PrometheusRuleRef).Times(1).Return(consterror.CustomError)
					})
				...
				When("the PrometheusRule deletion was successful", func() {
					BeforeEach(func() {
						mockUtils.EXPECT().DeletePrometheusRuleDeployment(gomock.Any(), gomock.Any(), routeMonitor.Status.PrometheusRuleRef).Times(1)
					})
		...
		Describe("The RouteMonitor settings are VALID", func() {
			BeforeEach(func() {
				mockUtils.EXPECT().AreMonitorSettingsValid(routeMonitor.Status.RouteURL, routeMonitor.Spec.Slo).Return(true, nil, "99.5").Times(1)
				mockUtils.EXPECT().SetErrorStatus(gomock.Any(), nil).Return(false)
			})
			When("the update the PrometheusRule failed", func() {
				BeforeEach(func() {
					mockUtils.EXPECT().UpdatePrometheusRuleDeployment(gomock.Any(), gomock.Any(), gomock.Any()).Return(consterror.CustomError)
				})
			...
			When("the update of the PrometheusRule succeded", func() {
				BeforeEach(func() {
					mockUtils.EXPECT().UpdatePrometheusRuleDeployment(gomock.Any(), gomock.Any(), gomock.Any())
				})
	...
	Describe("EnsureServiceMonitorResourceExists", func() {
		...
		When("The RouteUrl and ClusterID are not set", func() {
			BeforeEach(func() {
				...
				mockUtils.EXPECT().GetClusterID(gomock.Any()).Return("test-cluster-id")
			})
		...
		Describe("It updates the ServiceMonitor targeting the blackbox Exporter Namespace", func() {
			...
			When("the update of the ServiceMonitor fails", func() {
				BeforeEach(func() {
					mockUtils.EXPECT().UpdateServiceMonitorDeployment(gomock.Any(), gomock.Any(), gomock.Any()).Return(consterror.CustomError)
				})
			...
			When("the update of the ServiceMonitor is successfull", func() {
				BeforeEach(func() {
					mockUtils.EXPECT().UpdateServiceMonitorDeployment(gomock.Any(), gomock.Any(), gomock.Any())
				})
